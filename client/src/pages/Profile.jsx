...
import { useDispatch, useSelector } from 'react-redux' // useSelector for retrieving data of current user stored in local storage
...
export default function Profile() {
  ...
  //  allow read;
  //     allow write: if
  //     request.resource.size < 9 * 1024 * 1024 &&
  ...
  return (
    <div className='p-3 max-w-lg mx-auto'>
      ...
      <form onSubmit={handleSubmit} className='flex flex-col gap-4'>
        ...
        className='rounder-full h-24 w-24 object-cover cursor-pointer 
        self-center mt-2'/>
        <p className='text-sm self-center'>
          {fileUploadError ? (
            <span className='text-red-700'>
            Error Image Upload (image must be less than 9 mb)
          </span>
